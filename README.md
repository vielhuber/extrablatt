...
the `Crypto` tab shows the rolling one-year BTC/EUR and ETH/EUR price history from CoinGecko's public endpoint without a configured API key, using the bundled Chart.js library. market data stays fresh for 15 minutes, with the last cached response retained through temporary API failures; the weekly digest adds the same one-year movement as prose above the weather.
